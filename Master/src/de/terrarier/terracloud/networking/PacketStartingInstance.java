...
public class PacketStartingInstance extends Packet {
    ...
    protected void read0(DataContainer dataContainer, Connection connection) {
        ...
        final InstanceManager<?, ?> instanceManager = proxy ? Master.getInstance().getProxyManager()
                : Master.getInstance().getServerManager();
        final GroupInstance<?> group = instanceManager.getGroup(groupName);
        if(group != null) {
            group.addServer(id, Master.getInstance().getWrapperManager().fromConnection(connection));
        }
    }
