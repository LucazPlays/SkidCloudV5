...
public class PacketUpdateGroup extends Packet {
    ...
    protected void read0(DataContainer dataContainer, Connection connection) {
        ...
        switch (updateType) {
            case ATTRIBUTE:
                ...
                break;
            case CREATE:
                InstanceManager<?, ?> manager = proxy ? Master.getInstance().getProxyManager()
                        : Master.getInstance().getServerManager();
                manager.createGroup(createGroup(proxy, dataContainer));
                break;
            case DELETE:
                manager = proxy ? Master.getInstance().getProxyManager()
                        : Master.getInstance().getServerManager();
    ...
    private <G> G createGroup(boolean proxy, DataContainer dataContainer) {
