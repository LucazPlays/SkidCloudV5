    ...
    public void execute(String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            if (args.length != 6 && (args.length != 7 || !args[1].equalsIgnoreCase("-s"))
                    || !args[1].equalsIgnoreCase("-p") && !args[1].equalsIgnoreCase("-s")
                    || !TypeCheckUtil.isInteger(args[3]) || !TypeCheckUtil.isInteger(args[4])
                    || !args[5].equalsIgnoreCase("Dynamic") && !args[5].equalsIgnoreCase("Static")) {
                Logger.log("Folgende Parameter sind erforderlich \"" + "[-p/-s] [Name] [Servers] [Memory] [Dynamic,Static] " + Color.ITALIC + Color.YELLOW + "[ServerVersion]" + Color.RESET + "\"!", LogType.WARN);
                return;
            }
            boolean proxy = args[1].equalsIgnoreCase("-p");
            if (proxy) {
                if (Master.getInstance().getProxyManager().getGroup(args[2]) != null) {
                    Logger.log("Diese ProxyGruppe existiert bereits!", LogType.WARN);
                    return;
                }
                Master.getInstance().getProxyManager().createGroup(new ProxyGroup(args[2], Integer.parseInt(args[3]), args[5].equalsIgnoreCase("dynamic"), Integer.parseInt(args[4])));
                Logger.log("Die ProxyGruppe \"" + args[2] + "\" wurde erstellt!", LogType.INFO);
                return;
            }
            if (Master.getInstance().getServerManager().getGroup(args[2]) != null) {
            ...
            Master.getInstance().getServerManager().createGroup(new ServerGroup(args[2], Integer.parseInt(args[3]), args[5].equalsIgnoreCase("dynamic"), Integer.parseInt(args[4]), serverVersion));
            ...
            case "delete":
                ...
                final InstanceManager<?, ?> manager = proxy ?
                        Master.getInstance().getProxyManager() : Master.getInstance().getServerManager();
                final GroupInstance<?> grp = manager.getGroup(args[2]);
                if(grp == null) {
                    Logger.log("Diese Gruppe existiert nicht!", LogType.WARN);
                    return;
                }
                manager.deleteGroup(args[2]);
                Master.getInstance().broadcastPacket(new PacketUpdateGroup(args[2], proxy));
                break;
            case "list":
