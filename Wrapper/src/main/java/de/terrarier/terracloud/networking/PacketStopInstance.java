...
public class PacketStopInstance extends Packet {
    ...
    protected void read0(DataContainer dataContainer, Connection connection) {
        ...
        InstanceManager manager = proxy ? Wrapper.getInstance().getProxyManager() : Wrapper.getInstance().getServerManager();
        final GroupInstance<?> group = manager.getGroup(groupName);
        if(group != null) {
            final ServerInstance<?> server = group.getServer(id);
            if(server != null) {
                manager.stopServer(server, Wrapper.getInstance().getMasterConnection().getConnection().getId() == connection.getId());
            }
        }
    }
