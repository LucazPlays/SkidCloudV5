...
public final class DataCompressionUtil {
    ...
    public static boolean[] fromByte(byte compressed, int values) {
        ...
        final boolean[] tret = new boolean[Math.min(values, 7)];
        tret[0] = fromBit(compressed & 1);
        for(int i = 1; i < tret.length; i++) {
            tret[i] = fromBit((compressed << i) & 1);
        }
        return tret;
    }
